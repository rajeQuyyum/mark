...
export default function AdminDashboard() {
  ...
  const updateBalance = (userId) => {
    ...
    axios
      .put(`${API}/admin/user/${userId}/balance`, { balance })
      .then((res) =>
  ...
  const updateTransaction = (userId, txId) => {
    ...
    axios
      .put(`${API}/admin/transaction/${txId}`, tx)
      .then((res) => {
        const updatedTx = res.data;
        setTransactions((prev) => ({
          ...prev,
          [userId]: prev[userId].map((t) =>
            t._id === txId ? updatedTx : t
          ),
        }));
  ...
  const deleteTransaction = (userId, txId) => {
    ...
    axios
      ...
      .then(() => {
        setTransactions((prev) => ({
          ...prev,
          [userId]: prev[userId].filter((t) => t._id !== txId),
        }));
  ...
  const deleteSelectedUsers = () => {
    if (!window.confirm("Are you sure you want to delete selected users?"))
      return;
    axios
      .post(`${API}/admin/users/delete-multiple`, { ids: selectedUsers })
      .then(() => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <header className="bg-red-600 text-white p-4 flex justify-between flex-wrap">
        ...
        <button
          ...
          className="mt-2 sm:mt-0 bg-blue-500 px-3 py-1 rounded"
        >
          Logout
        </button>
      </header>
          ...
          {users.map((user) => (
            ...
            >
              ...
              <button
                ...
                className="bg-red-500 text-white px-2 py-1 mt-1 w-full"
              >
                Delete User
              </button>
              ...
              <hr className="my-2" />
              <button
                onClick={() => fetchTransactions(user._id)}
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewTransaction({
                      ...
                      [user._id]: {
                        ...newTransaction[user._id],
                        counterpartyAccount: e.target.value,
                      },
                    })
                  }
                />
              ...
              {(transactions[user._id] || []).map((tx) => (
                <div
                  key={tx._id}
                  className="border p-2 mb-2 mt-2 rounded bg-gray-100 flex flex-col"
                >
                  <input
                    className="border w-full mb-1"
                    value={
                      editingTransaction[tx._id]?.recipientName ??
                      tx.recipientName
                    }
                    onChange={(e) =>
                  ...
                  <input
                    className="border w-full mb-1"
                    value={
                      editingTransaction[tx._id]?.counterpartyAccount ??
                  ...
                  <input
                    className="border w-full mb-1"
                    value={
                      editingTransaction[tx._id]?.description ?? tx.description
                    }
                    onChange={(e) =>
                      setEditingTransaction({
                        ...editingTransaction,
                  ...
                  {/* ✅ NEW Delete Transaction Button */}
                  <button
                    onClick={() => deleteTransaction(user._id, tx._id)}
                    className="bg-red-500 text-white px-2 py-1 w-full mt-1"
                  >
                    Delete Transaction
                  </button>
                </div>
